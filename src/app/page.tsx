...
import { logAction } from '@/utils/logger'; // Make sure the path is correct
...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (token) {
      router.push("/dashboard"); // Redirect to dashboard if token is set
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("data.privileges" + JSON.stringify(data));

      if (res.ok) {
        sessionStorage.setItem("child_info", data.student);

        sessionStorage.setItem("role_name", data.role.role_name);
        sessionStorage.setItem("userid", data.userid);
        await logAction('User Login', data.userid);
