...
import exportToCsv from '@/lib/exportToCsv';
      ...
      const fetchStudents = async () => {
        const ChildId = sessionStorage.getItem("child_info");
        //     console.log(ChildId);
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/student-reports/${ChildId}`);
...
export default function ChildProgressReport(studentId) {
    ...
    return (
        <DefaultLayout>
            <Breadcrumb pageName="Child Progress Report" />
        ...
        data-tooltip-id="export-child -progress-report-tooltip"
        data-tooltip-content="Export Child Progress Report" 
        >
        Export Child Progress Report &nbsp;&nbsp;
        <svg className="w-5 h-5 inline-block"
                                ...
                                >
                                    ...
                                    <ReactTooltip id="export-child -progress-report-tooltip" place="top" effect="solid" />
                    ...
                    <tbody>
                        {students.map((student, index) => (
                            <tr key={student._id} className={`border-b border-stroke dark:border-strokedark`}>
                                ...
                                <td className="p-4 text-center text-black dark:text-white">{student.name}</td>
                                <td className="p-4 text-center text-black dark:text-white">{student.class}</td>
                                <td className="p-4 text-center text-black dark:text-white">{student.grade}</td>                                    
                            </tr>
                        ))}
                    </tbody>
                </table>
